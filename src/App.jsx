...
function App() {
  ...
  async function fetchPopulars() {
    try {
      ...
      if (popularShowsData.length > 0) {
        const upperBound = Math.min(popularShowsData.length, 10);
        const selectedIndex = Math.trunc(Math.random() * upperBound);

        setCurTvShow(popularShowsData[selectedIndex]); // take the currently most popular show
      }
    } catch (err) {
  ...
  async function fetchRecommendations(tvShowId) {
    try {
      const recommendedShowsData = await TvShowApi.fetchRecommendations(
        tvShowId
      );
  ...
  return (
    ...
    >
        ...
        <div className="row">
          ...
          <div className="col-4">
            <Logo
              img={logoImg}
              title="BingeBuddy"
              subtitle="Find a show suited for you!"
            />
          </div>
      ...
      <div className={s.recommended_tv_shows}>
        {recommendationList && (
          <TvShowList
            onClickItem={updateCurrentShow}
            tvShows={recommendationList}
          />
        )}
      </div>
    </div>
